        ...
        <div class="page-header">
            <div class="page-header-left d-flex align-items-center">
                <div class="page-header-title">
                    <h5 class="m-b-10">Edit Newsletter</h5>
                </div>
                <ul class="breadcrumb">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="subject" class="form-control @error('subject') is-invalid @enderror" value="{{ old('subject', $newsletter->subject) }}" placeholder="Enter subject line..." required>
                                    @error('subject')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Excerpt</label>
                                    <textarea name="excerpt" class="form-control @error('excerpt') is-invalid @enderror" rows="2" placeholder="Brief overview or preview snippet...">{{ old('excerpt', $newsletter->excerpt) }}</textarea>
                                    @error('excerpt')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <div class="mb-3">
                                    <label class="form-label required">Body Content</label>
                                    <textarea id="tinymce-editor" name="body" class="form-control @error('body') is-invalid @enderror" style="min-height: 400px;">{{ old('body', $newsletter->body) }}</textarea>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select name="audience" class="form-select @error('audience') is-invalid @enderror" required>
                                        ...
                                        @foreach($audiences as $key => $label)
                                            <option value="{{ $key }}" {{ old('audience', $newsletter->audience) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="url" name="featured_image_url" class="form-control @error('featured_image_url') is-invalid @enderror" value="{{ old('featured_image_url', $newsletter->featured_image_url) }}" placeholder="https://">
                                    @error('featured_image_url')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Featured Video URL</label>
                                    <input type="url" name="featured_video_url" class="form-control @error('featured_video_url') is-invalid @enderror" value="{{ old('featured_video_url', $newsletter->featured_video_url) }}" placeholder="https://">
                                    @error('featured_video_url')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <div class="mb-3 form-check">
                                    <input type="checkbox" class="form-check-input" name="show_on_blog" id="show_on_blog" value="1" {{ old('show_on_blog', $newsletter->show_on_blog) ? 'checked' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="action" id="action-select" class="form-select @error('action') is-invalid @enderror" required>
                                        <option value="draft" {{ old('action', $newsletter->status) == 'draft' ? 'selected' : '' }}>Save as Draft</option>
                                        <option value="send_now" {{ old('action', $newsletter->status) == 'send_now' ? 'selected' : '' }}>Send Now</option>
                                        <option value="schedule" {{ old('action', $newsletter->status) == 'schedule' ? 'selected' : '' }}>Schedule</option>
                                    </select>
                                </div>

                                <div class="mb-3 d-none" id="schedule-container">
                                    <label class="form-label">Scheduled Date/Time</label>
                                    <input type="datetime-local" name="scheduled_at" class="form-control @error('scheduled_at') is-invalid @enderror" value="{{ old('scheduled_at', isset($newsletter->scheduled_at) ? \Carbon\Carbon::parse($newsletter->scheduled_at)->format('Y-m-d\TH:i') : '') }}">
                                    @error('scheduled_at')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <button type="submit" class="btn btn-primary w-100 mt-2">
                                    <i class="feather-save me-1"></i> Update Newsletter
                                </button>
                            </div>
                        </div>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tinymce.init({
            ...
            // Handle automatic Cloudinary file uploads
